...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["audio/*"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for audio-recordings" ON storage.objects FOR SELECT USING (bucket_id = 'audio-recordings');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for audio-recordings" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'audio-recordings');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for audio-recordings" ON storage.objects FOR UPDATE USING (bucket_id = 'audio-recordings');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for audio-recordings" ON storage.objects FOR DELETE USING (bucket_id = 'audio-recordings');`
    ];
